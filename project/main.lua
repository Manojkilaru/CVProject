...
--baseline - https://github.com/kaishengtai/neuralart
...
local original = image.load(opt.content):cuda()
print(original:size())
-- original = original:reshape(original:size(2),original:size(3),original:size(4))
model:forward(img)
...
function opfunc(input)
    ...
    for i = #model.modules, 1, -1 do
        ...
        --[[--]]w1,_=module:getParameters()
        --[[if(w1:empty()) then
            print(w1[20])
        end--]]
        grad = module:backward(module_input, grad)
        --[[w2,_=module:getParameters()
        if(w1) then
            print("-------------------------------")
            print(w2[20])
        end--]]
...
for i = 1, opt.num_iters do
    print(i)
    local input_, loss = optim[opt.optimizer](opfunc, input, optim_state)
    loss = loss[1]
    ...
    if i <= 20 or i % 5 == 0 then
        output = depreprocess(input):double():cuda()
        --[[if opt.display_interval > 0 and i % opt.display_interval == 0 then
            image.display(output)
        end--]]
        mask = image.load("out1.jpg"):cuda()
        ones = torch.ones(mask:size()):cuda()
        mask1 = ones - mask
        -- mask1 = image.load("out2.jpg"):cuda()
        mask = mask:expand(3,mask:size(2),mask:size(3))
        mask1 = mask1:expand(3,mask1:size(2),mask1:size(3))
        final = torch.cmul(mask,output)+torch.cmul(mask1,original)
        image.save(paths.concat(frames_dir, string.format('%04d.jpg',i)), final)
    end
    local out2 = model:forward(img)
    -- if(out1==out2) then 
    --     print("lol")
    -- end
end
